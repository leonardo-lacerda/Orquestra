---
name: orquestra-theme
description: Author or edit a Orquestra IDE theme — one data-driven theme covering app chrome colors, the terminal ANSI palette, and Monaco editor syntax tokens. Use when the user asks to create, customize, recolor, or generate a Orquestra theme, mentions a color scheme (Dracula, Nord, "make Orquestra look like X"), or wants a new light/dark theme.
user-invocable: true
---

# Authoring Orquestra Themes

A Orquestra theme is ONE JSON object that styles the whole IDE:
...
User themes are stored in Orquestra's settings file under the `customThemes` array,
and the active theme is `activeThemeId`:

```
macOS:   ~/Library/Application Support/orquestra/config.json
Linux:   ~/.config/orquestra/config.json
Windows: %APPDATA%/orquestra/config.json
```
...
Save the theme as `<name>.orquestra-theme.json` and tell the user:
