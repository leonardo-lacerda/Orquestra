...
const RESULTS_DIR = '.orquestra-results'
...
function send(cmd, args = {}) {
  ...
  const maestroId = readCrownMaestroId()
...
function resultPathFor(name) {
  return path.resolve(RESULTS_DIR, `worker-${safeWorkerFileName(name)}.json`)
}

function readWorkerResult(name) {
  const rp = resultPathFor(name)
...
if (!cmd || cmd === '--help' || cmd === '-h') {
  ...
  print('')
  print('Examples:')
  print('  orquestra recruit --name api --role "Implement the API handlers only"')
  print('  orquestra wait --workers api,ui --timeout 300')
  print('  orquestra plan --file plan.json')
  print('  orquestra status')
  print('  orquestra dismiss api')
  process.exit(0)
}
...
switch (cmd) {
  case 'recruit': {
    ...
    if (!flags.name) {
      ...
      print('Usage: orquestra recruit --name <function-id> --role "short unique task"')
      process.exit(1)
    }
    if (!flags.role) {
    ...
    print(`Recruiting function ${flags.name}: ${flags.role}${flags.agent ? ` (${flags.agent})` : ''}`)
    break
  }
  ...
  case 'list': {
    send('list')
    const resultDir = path.resolve(RESULTS_DIR)
  ...
  case 'wait': {
    ...
    while (Date.now() < deadline) {
      const snapshots = workerNames.map((name) => ({ name, result: readWorkerResult(name) }))
    ...
    for (const name of workerNames) {
      const result = readWorkerResult(name)
      if (!result || !isTerminalStatus(result.status)) {
        ...
        try {
          ...
          fs.writeFileSync(resultPathFor(name), JSON.stringify({
            ...existing,
            name,
            workerName: name,
  ...
  case 'plan-status': {
    ...
    const resultsDir = path.resolve(RESULTS_DIR)
