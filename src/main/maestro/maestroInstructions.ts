  ...
  return [
    ...
    ...(runId
      ? [
          ...
          `Example wait: node .orquestra/cli/orquestra.cjs wait --run ${runId} --workers name1,name2 --timeout 300`,
          `(CLI lives under .orquestra/cli/ — always use orquestra.cjs; immune to package.json "type":"module")`,
        ]
      : multi
        ? [
            '',
            ...
            'On every CLI call use: node .orquestra/cli/orquestra.cjs … --run %ORQUESTRA_RUN_ID%   (cmd)',
            '  or: node .orquestra/cli/orquestra.cjs … --run $env:ORQUESTRA_RUN_ID   (PowerShell)',
            '  or: node .orquestra/cli/orquestra.cjs … --run "$ORQUESTRA_RUN_ID"   (bash)',
            ...
            'Example: node .orquestra/cli/orquestra.cjs wait --run <YOUR_ORQUESTRA_RUN_ID> --workers w1 --timeout 300',
          ]
        : []),
    '',
    ...
    '  node .orquestra/cli/orquestra.cjs recruit --name w1 --role "short task for this step"',
    ...
    '  - Prefer: node .orquestra/cli/orquestra.cjs reassign <name> --role "next short task"',
    ...
    '',
    'Command:',
    '  node .orquestra/cli/orquestra.cjs dismiss <name>',
    '',
    ...
    '',
    '  node .orquestra/cli/orquestra.cjs recruit --name <slot-or-label> --role "<short task for THIS step only>"',
    '  node .orquestra/cli/orquestra.cjs reassign <slot-or-label> --role "<next task on same worker>"',
    '  node .orquestra/cli/orquestra.cjs dismiss <slot-or-label>',
    '',
    'Preferred pool loop (serial on one slot — default for one feature):',
    '  node .orquestra/cli/orquestra.cjs recruit --name w1 --role "Implement the core module only."',
    '  node .orquestra/cli/orquestra.cjs wait --workers w1 --timeout 300',
    '  node .orquestra/cli/orquestra.cjs reassign w1 --role "Add tests for the happy path and one failure."',
    '  node .orquestra/cli/orquestra.cjs wait --workers w1 --timeout 300',
    '  node .orquestra/cli/orquestra.cjs reassign w1 --role "Write a short usage note in docs/."',
    '  node .orquestra/cli/orquestra.cjs wait --workers w1 --timeout 180',
    '  node .orquestra/cli/orquestra.cjs dismiss w1',
    '',
    'Parallel only when independent (≤ pool size):',
    '  node .orquestra/cli/orquestra.cjs recruit --name w1 --role "…"',
    '  node .orquestra/cli/orquestra.cjs recruit --name w2 --role "…"',
    '  node .orquestra/cli/orquestra.cjs wait --workers w1,w2 --timeout 300',
    '',
    ...
    'JSON: .orquestra/runs/<runId>/results/worker-<name>.json (or .orquestra/results/ when flat).',
    'When done consolidating: dismiss workers you will not reassign.',
    '',
    '## COMMANDS REFERENCE',
    '',
    '  node .orquestra/cli/orquestra.cjs recruit --name <function-id> --role "short task"',
    '  node .orquestra/cli/orquestra.cjs reassign <name> --role "follow-up task"',
    '  node .orquestra/cli/orquestra.cjs dismiss <name>          # close that worker panel',
    '  node .orquestra/cli/orquestra.cjs wait --workers n1,n2 --timeout 300',
    '  node .orquestra/cli/orquestra.cjs status | list',
    '',
  ].join('\n')
}
